...
function setupCanvas() {
    ...
    // En entorno de pruebas, usar un canvas mock
    if (simulationState.isTestEnvironment) {
        console.log("Entorno de pruebas: usando canvas simulado");
        canvas = document.createElement('canvas'); // Crear un canvas real pero no añadir al DOM
        
        // Crear un contexto real para pruebas
        try {
            ctx = canvas.getContext('2d');
        } catch (error) {
            // Si falla, crear un contexto simulado
            ctx = {
                clearRect: () => {},
                fillRect: () => {},
                strokeRect: () => {},
        ...
        // Asignar dimensiones ficticias para pruebas
        canvas.width = 800;
        canvas.height = 600;
...
function setupEventListeners() {
    ...
    // Cambio en el modelo de amplificación
